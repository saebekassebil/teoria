...
var TeoriaNote = require('./note');
var TeoriaInterval = require('./interval');
...
function TeoriaChord(root, name) {
  ...
  for (i = 0, length = name.length; i < length; i++) {
    ...
    switch (parsing) {
      ...
      case 'quality':
        ...
        symbol = (shortQ in knowledge.kSymbols) ?
          shortQ : (c in knowledge.kSymbols) ? c : '';

        setChord(knowledge.kSymbols[symbol]);
  ...
  this.intervals = notes
    ...
    .concat(additionals)
    .map(function(i) { return TeoriaInterval.toCoord(i); });
  ...
  if (bass) {
    ...
    note = TeoriaNote.fromString(bass + (root.octave() + 1)); // crude

    bassInterval = TeoriaInterval.between(root, note);
...
TeoriaChord.prototype = {
  ...
  get: function(interval) {
    if (typeof interval === 'string' && interval in knowledge.kStepNumber) {
      var intervals = this.intervals, i, length;

      interval = knowledge.kStepNumber[interval];
