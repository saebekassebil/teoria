  ...
  'Teoria Scales': {
    'Ab2': {
      ...
      'Ionian/Major': function(note) {
        assert.deepEqual(note.scale('ionian', true),
            ['ab', 'bb', 'c', 'db', 'eb', 'f', 'g']);
      },

      'Dorian': function(note) {
        assert.deepEqual(note.scale('dorian', true),
            ['ab', 'bb', 'cb', 'db', 'eb', 'f', 'gb']);
      },

      'Phrygian': function(note) {
        assert.deepEqual(note.scale('phrygian', true),
            ["ab", "bbb", "cb", "db", "eb", "fb", "gb"]);
      },

      'Lydian': function(note) {
        assert.deepEqual(note.scale('lydian', true),
            ["ab", "bb", "c", "d", "eb", "f", "g"]);
      },

      'Mixolydian': function(note) {
        assert.deepEqual(note.scale('mixolydian', true),
            ["ab", "bb", "c", "db", "eb", "f", "gb"]);
      },

      'Aeolian/Minor': function(note) {
        assert.deepEqual(note.scale('aeolian', true),
            ["ab", "bb", "cb", "db", "eb", "fb", "gb"]);
      },

      'Locrian': function(note) {
        assert.deepEqual(note.scale('locrian', true),
            ["ab", "bbb", "cb", "db", "ebb", "fb", "gb"]);
      },

      'Major Pentatonic': function(note) {
        assert.deepEqual(note.scale('majorpentatonic', true),
            ["ab", "bb", "c", "eb", "f"]);
      },

      'Minor Pentatonic': function(note) {
        assert.deepEqual(note.scale('minorpentatonic', true),
            ["ab", "cb", "db", "eb", "gb"]);
      },

      'Chromatic': function(note) {
        assert.deepEqual(note.scale('chromatic', true),
            ["ab", "bbb", "bb", "cb", "c", "db",
             "d", "eb", "fb", "f", "gb", "g"]);
      }
    }
